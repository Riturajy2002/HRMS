...
public class ExcelReportService {
	...
	public byte[] generateMonthlyReport(YearMonth yearMonth) throws IOException {
		...
		try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			String[] headers = { " Emp ID ", "User Name", "Available Leaves", "Approved Leaves", "Declined Requests",
					"Flexi Count", "LWP Count", "Year Month" };
			...
			for (MonthlyReport report : reports) {
				...
				row.createCell(2).setCellValue(report.getAvailableLeaves());
				row.createCell(3).setCellValue(report.getApprovedLeaves());
				row.createCell(4).setCellValue(report.getDeclinedRequests());
				row.createCell(5).setCellValue(report.getPendingLeaves());
